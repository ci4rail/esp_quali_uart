...
esp_err_t new_uart_test(quali_uart_test_handle_t **hdl_p, quali_uart_test_config_t *test_config)
{
    ...
    *hdl_p = NULL;
    ...
    /* setup uart driver */
    if (uart_driver_install(test_config->uart_num, test_config->rx_buf_size, test_config->tx_buf_size, 0, NULL,
            INTR_ALLOC_FLAGS) != ESP_OK) {
    ...
    if (uart_set_pin(test_config->uart_num, test_config->gpio_tx, test_config->gpio_rx, test_config->gpio_rts,
            test_config->gpio_cts) != ESP_OK) {
    ...
    if (xTaskCreate(uart_test_control, control_task_name, TASK_STACK_SIZE, (void *)hdl, 10, &hdl->control_task) !=
        pdPASS) {
